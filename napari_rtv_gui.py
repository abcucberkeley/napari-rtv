...
class dataAuditorTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Input fields
        self.folder_paths_entry = self.add_input_field(layout, "Folder Paths (comma-separated)", self.default_values['folder_paths'], 1, True)
        layout.addStretch(1)
        self.channel_patterns_entry = self.add_input_field(layout, "Channel Patterns (comma-separated)", self.default_values['channel_patterns'], 2)
        layout.addStretch(1)
        self.voxel_resolution_entry = self.add_input_field(layout, "Voxel Resolution (z,y,x in nm)", self.default_values['voxel_resolution'], 3)
        layout.addStretch(1)
        self.timepoint_range_entry = self.add_input_field(layout, "Timepoint Range (start or start,end)", self.default_values['timepoint_range'], 4)
        layout.addStretch(1)
        self.timepoint_step_size_entry = self.add_input_field(layout, "Timepoint Step Size", self.default_values['timepoint_step_size'], 5)
        layout.addStretch(1)
        self.max_timepoints_entry = self.add_input_field(layout, "Max Timepoints (inf or a whole number)", self.default_values['max_timepoints'], 6)
    ...
    def add_input_field(self, layout, label_text, default_value, row, has_browse=False):
        hboxlayout = QHBoxLayout()
